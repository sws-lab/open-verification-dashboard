(** Set of functions used to analyse the project structure to match paths
    against it *)
  ...
  | Directory of string * bool ref * project_structure Folder.t
      (** Represents the structure of the project.
          ...
          - [Directory (name, scanned, files)]: a directory with the given name,
            a boolean indicating if it has been scanned, and a map of files and
            directories. *)

let rec pp_project_structure fmt = function
  | File path -> Format.fprintf fmt "File: %s" path
  | Directory (name, scanned, files) ->
      if not !scanned then Format.fprintf fmt "%s (not scanned)" name
      else
        Format.fprintf fmt "%s (scanned) @[<v 0>@,%a@]" name
          (Format.pp_print_list ~pp_sep:Format.pp_print_cut pp_project_structure)
...
let init project_path =
  if !base_path = "" then base_path := project_path
  else assert (!base_path = project_path)
  ...
  if not !scanned then (
    ...
    Array.iter
      (fun f ->
        ...
        else Folder.replace files f (File f))
      folder_content)

(** Tries to match the given path against the project structure. The function
    dynamically scans directories as needed. *)
let match_path (project_path : string) (path : string list) =
  let rec aux structure matched_path path acc =
    match (path, structure) with
    | [], _ -> acc
    | "." :: rest, _ -> aux structure matched_path rest acc
    | ".." :: rest, _ -> (
        match acc with
    ...
    | file :: [], Directory (_, scanned, files) -> (
        ...
        | Some (File file_path) -> file_path :: acc
        | Some (Directory _) -> []
        | None -> [])
    | dir :: rest, Directory (_, scanned, files) -> (
        ...
        | None -> if acc = [] then aux structure matched_path rest acc else [])
    | _ -> []
  in
...
(** Converts a file path to a project-relative path. If the file is not found in
    the project structure, it prints a warning and returns the original path. If
    [warn] is false, it exits with an error if the file is not found. The
    function initializes the project structure and scans it dynamically as
    needed. *)
...
let path_to_project_relative ?(warn = true) project_path file_path =
  ...
  | None ->
      ...
      let relative_path =
        path_to_project_relative__ ~warn project_path file_path
      in
